 ...
 * NOTE: AmazonQuirks removed - not available in standard Media3
 */
public class CustomOverridesRenderersFactory extends CustomRenderersFactoryBase {
    ...
    public CustomOverridesRenderersFactory(Context activity) {
        ...
        // AmazonQuirks removed - these were Amazon Fire TV specific tweaks
        // that are not available in standard Media3
    }
    ...
    // Exo 2.9
    // @Override
    // protected void buildAudioRenderers(Context context, int
    // extensionRendererMode, MediaCodecSelector mediaCodecSelector,
    // @Nullable DrmSessionManager<ExoMediaDrm> drmSessionManager, boolean
    ...
    // audioRenderer =
    // new CustomMediaCodecAudioRenderer(context, mediaCodecSelector,
    // drmSessionManager, playClearSamplesWithoutKeys, eventHandler,
    // eventListener, new
    // DefaultAudioSink(AudioCapabilities.getCapabilities(context),
    // audioProcessors));
    //
    ...
    // Exo 2.9
    // @Override
    // protected void buildVideoRenderers(Context context, int
    // extensionRendererMode, MediaCodecSelector mediaCodecSelector,
    // @Nullable DrmSessionManager<ExoMediaDrm> drmSessionManager, boolean
    // playClearSamplesWithoutKeys,
    // Handler eventHandler, VideoRendererEventListener eventListener, long
    ...
    //
    // if (mPlayerTweaksData.isFrameDropFixEnabled() ||
    // mPlayerTweaksData.isAmlogicFixEnabled()) {
    // videoRenderer = new CustomMediaCodecVideoRenderer(context,
    // mediaCodecSelector, allowedVideoJoiningTimeMs, drmSessionManager,
    ...
    // Media3 version - simplified API
    // NOTE: Media3 DefaultRenderersFactory.buildAudioRenderers has different
    // signature
    // For now, we'll skip custom audio renderers since the API changed
    // significantly
    // TODO: Update to Media3 audio renderer API if audio delay is needed

    // Media3 version - simplified API
    // NOTE: Media3 DefaultRenderersFactory.buildVideoRenderers has different
    // signature
    // For now, we'll skip custom video renderers since the API changed
    // significantly
    // TODO: Update to Media3 video renderer API if frame drop fixes are needed

    // Old ExoPlayer 2.10/2.11 buildVideoRenderers code - commented out for Media3
    // if (!mPlayerTweaksData.isAmazonFrameDropFixEnabled() &&
    // !mPlayerTweaksData.isSonyFrameDropFixEnabled()
    // && !mPlayerTweaksData.isAmlogicFixEnabled()) {
    // DebugInfoMediaCodecVideoRenderer videoRenderer = new
    // DebugInfoMediaCodecVideoRenderer(...);
    // videoRenderer.enableSetOutputSurfaceWorkaround(true);
    // replaceVideoRenderer(out, videoRenderer);
    // return;
    // }
    // TweaksMediaCodecVideoRenderer videoRenderer = new
    // TweaksMediaCodecVideoRenderer(...);
    // videoRenderer.enableFrameDropFix(...);
    // replaceVideoRenderer(out, videoRenderer);
    ...
    //
    // if (mPlayerData.getAudioDelayMs() == 0) {
    // // Improve performance a bit by eliminating calculations presented in custom
    // renderer.
    //
    // return;
    // }
    ...
    // if (!mPlayerTweaksData.isFrameDropFixEnabled() &&
    // !mPlayerTweaksData.isAmlogicFixEnabled()) {
    // // Improve performance a bit by eliminating some if conditions presented in
    // tweaks.
